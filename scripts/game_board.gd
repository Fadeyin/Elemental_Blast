...
# Идёт стартовая сцена метеоритного дождя — блокируем ходы до завершения
...
# Предуровневый «Метеоритный дождь»: тайминг падения и пауза между ударами по клеткам
...
func _draw():
	...
	for vfx in _board_vfx:
		...
		match vfx.type:
			...
			"meteor_strike":
				var kk := clampf(vfx.t / vfx.d, 0.0, 1.0)
				...
				draw_line(tail + dir_m.orthogonal() * 2.0, head, Color(1.0, 0.92, 0.75, 0.45 * (1.0 - kk)), 3.0)
...
func _run_meteor_rain_intro() -> void:
	set_process(true)
	var strikes: Array = []
	for y in range(ENEMY_ROWS):
		for x in range(COLS):
	...
	for st in strikes:
		...
		await get_tree().create_timer(METEOR_RAIN_FALL_DURATION).timeout
		_apply_meteor_hit_to_cell(st.x, st.y)
		_update_ui()
		queue_redraw()
		await get_tree().create_timer(METEOR_RAIN_CELL_STAGGER_SEC).timeout
