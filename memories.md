...
- Level end: victory requires cleared targets/obstacles/queue/enemies; defeat on monster breach past heart row. Enemy zone always `ENEMY_ROWS` (10) rows; `_heart_row_y` = 9. Hearts in strip between zones; heart-kill uses `in_heart_strip` for death anim Y.
